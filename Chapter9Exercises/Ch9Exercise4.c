/* Chapter-9 Exercise-4 date and time update
* Vasanth [21 september 2014] */
...
int main(void)
{
	...
	printf("Time after one second is [HH:MM:SS] %.2i:%.2i:%.2i\n", nexttime.hour, nexttime.minutes, nexttime.seconds);
...
struct datetime datetimeupdate(struct datetime now, struct datetime date)
{
	...
	if (now.minutes == 60)
	{
		...
		if(now.hour == 24)
		{
			...
			printf("Date after one second is [DD:MM:YYYY] %.2i:%.2i:%.2i\n", nextdate.day, nextdate.month, nextdate.year);
		}
	}
